...
describe("HelpCommand", () => {
	...
	describe("help formatting", () => {
		it("should use consistent formatting across all help sections", () => {
			...
			// Check that we have colored output
			const hasColoredContent = calls.some((call) =>
				call.some((arg: any) => typeof arg === "string" && arg.includes("\u001b")),
			)
			expect(hasColoredContent).toBe(true)
		})
